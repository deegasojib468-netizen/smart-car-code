...
#if CAMERA_DIAG_TEST_IMAGE
            for(uint16 image_y = 0; image_y < MT9V03X_H; image_y++)
            {
                for(uint16 image_x = 0; image_x < MT9V03X_W; image_x++)
                {
                    image_copy[image_y][image_x] = (uint8)(image_x * 255 / (MT9V03X_W - 1));
                }
            }
#else
...
//      如果是使用逐飞科技 DAP-LINK 调试下载器连接，那么检查下载器线是否松动，检查核心板串口跳线是否已经焊接，串口跳线查看核心板原理图即可找到
